...
if($this->startResultCache(false, false, ($arParams["MENU_CACHE_USE_USERS"] === "Y"? $this->getGenerationCachePath($USER->GetID()): false)))
{
	...
	if ($arParams["MAX_LEVEL"] > 1)
	{
        ...
        $depth_sections_level = $arParams["MAX_LEVEL"];
	}
	else
	{
        ...
        $depth_sections_level = 1;
	}
    $sections_list = CIBlockSection::GetList (
        array(), 
        array(
            "IBLOCK_ID" => $arParams["IBLOCK_ID"], 
            "<=DEPTH_LEVEL" => $depth_sections_level
        ), 
        false, 
        array("IBLOCK_ID", "DEPTH_LEVEL", "ID", "NAME", "IBLOCK_SECTION_ID"), 
        false
    );
    while ($sections = $sections_list -> Fetch()) {
        if ($sections["DEPTH_LEVEL"] == 1){
            $arResult["SECTIONS"][$sections["ID"]] = $sections;    
        } else {
            $arResult["SECTIONS"][$sections["IBLOCK_SECTION_ID"]]["SUBSECTIONS"][$sections["ID"]] = $sections;
        }    
    }
    
    foreach ($arResult["SECTIONS"] as $arSection) {
        foreach ($arSection["SUBSECTIONS"] as $arSubsection) {
            if (strlen($arSubsection["IBLOCK_SECTION_ID"]) > 0) {
                $arResult["SECTIONS"][$arSubsection["IBLOCK_SECTION_ID"]]["IS_PARENT"] = "Y";
            }    
        }
    }
    $arResult["TOP_ELEMENTS"] = array();
    $elements_list = CIBlockElement::GetList (
        array("show_counter" => "asc"),
        array("IBLOCK_ID" => $arParams["IBLOCK_ID"]),
        false,
        false,
        array("ID", "NAME", "SECTION_ID")
    );
    while ($elements = $elements_list -> Fetch()) {
        $arResult["TOP_ELEMENTS"][$elements["SECTION_ID"]] = $elements["NAME"];    
    }
